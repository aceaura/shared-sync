...
- **T0/T1(Nebula)**:解析 nebula hostmap 判定当前是直连(underlay endpoint = peer 公网映射)还是经 relay(underlay = lighthouse),并在 overlay 内对数据中心做轻量心跳量 RTT/丢包。
...
- ⏳ Phase1b:VPS netns 可控双 NAT 验证 **T0 直连打洞**(real Linux,补 Phase0 在 Docker Desktop 下的缺口)。
- ⏳ Phase2:connd 接真 Nebula(hostmap 探测 T0/T1)+ 本地代理端点 + 三层状态机实跑自动升降级。
- ⏳ Phase3:T2 TCP 反向隧道(VPS frps + 数据中心 frpc)+ 并入阶梯兜底。
